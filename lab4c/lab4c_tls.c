...
void * getData() {
	...
	for (;;) {
		if (iflag == 0)
			continue;
		int readIn = SSL_read(s, buffer, 1024);
		if (readIn <= -1) exit(1);
		else if (readIn >= 1) {
			int i = 0;
			int given = 0;
...
int main(int argc, char **argv) {
	...
	bcopy((char *)server->h_addr,(char *)&addy.sin_addr.s_addr, server->h_length);
	...
	if (SSL_library_init() <= -1) {
		fprintf(stderr, "ERROR: failed to access SSL library.\n");
		exit(1);
	}

	const SSL_METHOD *a = SSLv23_client_method();
	SSL_CTX *c;
	if ((c = SSL_CTX_new(a)) == NULL) {
	...
	if (SSL_connect(s) < 1) {
		fprintf(stderr, "ERROR: failed to establish session with %s.\n", hostName);
		exit(1);
	}

	X509 *certificate = SSL_get_peer_certificate(s);
	if (certificate == 0) {
		fprintf(stderr, "ERROR: failed to read certificate %s.\n", hostName);
		exit(1);
	}
	...
	strcat(wID, ID);
	strcat(wID, "\n");
	int count = sprintf(wID, "ID=%s\n", ID);

	SSL_write(s, wID, count);
